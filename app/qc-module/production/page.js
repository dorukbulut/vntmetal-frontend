...
export default function Page() {
    ...
    const handleQCChange = async (product_id, qcValue) => {
        console.log("clicked")
        try {
            const res = await QCService.setQC(product_id, qcValue);
            if (res.status === 200) {
                setError({
                    isOpen: true,
                    type: "success",
    ...
    const { data1 } = useSWR(() => {
        if (filters) {
            ...
            QCService.getFilteredData(params)
                .then((res) => {
                    if (res.status === 200) {
                        const new_data = {
                            ...
                            rows: res.data.rows.map((item, index) => {
                                return {
                                    reference: item.productheader.reference,
                                    step: item.step,
                                        ...
                                        <div className="">
                                            <FormControl variant="standard" fullWidth>
                                                ...
                                                <Select sx={{ padding: "2px" }}>
                                                    <div className="p-2">
                                                        <Button
                                                            onClick={(e, product_id = item.product_id, qcValue="accepted") =>
                                                                handleQCChange(product_id, qcValue)
                                                            }
                                                            variant="outlined"
                                                            color={"success"}
                                                        >
                                                    ...
                                                    <div className="p-2">
                                                        <Button
                                                            onClick={(e, product_id = item.product_id, qcValue="pending") =>
                                                                handleQCChange(product_id, qcValue)
                                                            }
                                                            variant="outlined"
                                                            color={"warning"}
                                                    ...
                                                    <div className="p-2">
                                                        <Button
                                                            onClick={(e, product_id = item.product_id, qcValue="rejected") =>
                                                                handleQCChange(product_id, qcValue)
                                                            }
                                                            variant="outlined"
                                                            color={"error"}
            ...
            QCService.getPage(page).then((res) => {
                if (res.status === 200) {
                    const new_data = {
                        count: res.data.count,
                        rows: res.data.rows.map((item, index) => {
